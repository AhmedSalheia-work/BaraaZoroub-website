...
class Template
{
    ...
    private function renderFooterResources(){
        ...
        if (!array_key_exists('footer', $this->_template_parts)){
            ...
            if (!empty($js)){
                foreach ($js as $jsKey => $file){
                    if (isset($_SESSION['admin']) && explode(' ',str_replace(['\\','/'],' ',str_replace(array('C:\xampp\htdocs\BaraaZoroub-website\app\config\..\views\\','/home/progwlfo/baraazoroub.com/app/config/../views/'),'',$this->_action_view)))[0] == 'dashboard' && $jsKey == 'script'){
